...
| **Resend**         | 이메일    | 선택적으로 발송할 수 있는 구독자 대상 일일 이메일 다이제스트입니다. |
...
- **선택 이메일 다이제스트:** 구독자는 새로 게시된 글을 모은 일일 다이제스트 이메일을 받아볼 수 있습니다.
...
5. `apps/web/src/site.config.ts`의 `CONFIG.notify.fromAddress`를 3단계에서 인증한 도메인을 사용하는 `이름 <user@your-verified-domain>` 형식의 주소로 설정합니다. 발신자 정보는 모든 메일의 From 헤더에 이미 공개되는 브랜딩 정보이므로 env var가 아니라 커밋되는 설정 파일에 있습니다 — 자세한 이유는 해당 파일의 주석을 참고하세요.
   **이 필드는 기본값 여부를 검사하는 fail-closed 게이트가 없습니다 — 게이트는 값이 비어 있는지만 확인합니다. 템플릿 제작자의 기본 발신자 값을 그대로 두면 notify route는 멈추지 않고, 본인이 통제하지 않는 발신자/도메인으로 그대로 메일을 발송합니다.**
