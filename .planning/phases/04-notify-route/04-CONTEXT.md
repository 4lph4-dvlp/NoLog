...
- **D-06:** ~~The required physical mailing address is a **new field in `apps/web/src/site.config.ts`**~~ — matching the existing `profile`/`sns` configuration pattern. It's public information, not a secret, so there's no reason to make it an env var like `RESEND_API_KEY`. — **Reversibility:** reversible — moving it to an env var later is a config-source swap, not a data-contract change.
  - **Revised 2026-07-27 (post-execution, user-requested):** Moved to the `NOTIFY_PHYSICAL_ADDRESS` env var instead. Rationale: `site.config.ts` is committed to git, and NoLog is a fork-and-deploy template whose forks are typically public repos — a config-file field means a forker's real mailing address is permanently visible to anyone browsing the source, not just to people who subscribed and received the email in their inbox. An env var keeps the value out of source control while the digest footer's actual content is unchanged (the address still appears in every sent email either way; only its *storage location* changed). `fromAddress` stays in `site.config.ts` — it appears in every email's From header regardless, and the domain it points at is already public via DNS, so there is no equivalent exposure reduction from moving it. Implemented directly (not via `/gsd-plan-phase --gaps`) since it only touches the already-shipped 04-01 files; see `04-01-SUMMARY.md`'s post-execution revision note.
