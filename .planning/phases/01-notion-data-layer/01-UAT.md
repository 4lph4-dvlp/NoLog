...
expected: |
  Run `npx tsx packages/core/scripts/verify-phase-1.ts` against a live Notion test DB whose
  Status property uses the canonical capital name. stdout ends with "PASS: post correctly
  excluded after markEmailed()" — and, unlike the prior verification pass, the query itself
  should no longer 400 since CR-01 (property-key casing) is now closed.
awaiting: user response

## Tests

### 1. Mark-then-requery (DATA-01/DATA-02, ROADMAP SC#1-3)
expected: Run `npx tsx packages/core/scripts/verify-phase-1.ts` against a live Notion test DB whose Status property uses the canonical capital name (NOTION_TOKEN + NOTION_DATABASE_ID set, Emailed checkbox property present, ≥1 Status=public/Emailed-unchecked post). stdout ends with "PASS: post correctly excluded after markEmailed()", proving the query no longer 400s and the PATCH write durably persists and is visible on the immediately-following query.
result: [pending]
